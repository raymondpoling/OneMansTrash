...
object OfferDatabase {
    ...
    fun putData(offer: Offer) {
        ...
        val addId = Firebase.database
                .getReference(OfferDatabase.makePath(offer.seller, OFFERS)).push()
...
//        getData(orderRef, offerData)
    ...
    fun getWatchData() : LiveData<List<Offer>> {
        ...
        val orderRef = Firebase.database.getReference(makePath(user, WATCHERS))
    ...
    fun setWatch(offer:Offer) {
        ...
        Firebase.database.getReference(makePath(user, WATCHERS, Uri.parse(offer.id).lastPathSegment.toString())).setValue(offer)
    ...
    fun deleteWatch(offer:Offer) {
        val user = Firebase.auth.currentUser?.uid?:""
        debug("offer id is: ${offer.id}")
        Firebase.database.getReference(makePath(user, WATCHERS, Uri.parse(offer.id).lastPathSegment.toString())).removeValue()
